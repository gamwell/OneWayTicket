...
// ✅ Correction de l'import : le fichier est à la racine de src
...
// ✅ Type mis à jour (totalUsers supprimé pour plus de légèreté)
type Stats = {
  totalEvents: number;
  totalTickets: number;
  totalRevenue: number;
...
const AdminDashboardPage = () => {
  ...
  // ✅ Liste des cartes de statistiques (5 colonnes)
  const statCards = [
    {
      label: "Événements",
  ...
  // ✅ Liste des actions prioritaires (4 boutons bien alignés)
  const actions = [
    {
      to: "/admin/events/new",
      icon: Plus,
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {actions.map((action) => (
                <Link
                  key={action.label}
                  to={action.to}
                  className={`border p-6 rounded-3xl transition-all flex items-center gap-5 group ${colorMap[action.color]}`}
                >
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-6 h-fit">
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                <p className="text-center text-white/20 py-8 text-sm italic">Aucune vente récente</p>
              ) : (
                  ...
                  <div key={ticket.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-white/30 uppercase tracking-tighter">Vendu</p>
                      </div>
                    </div>
              ...
              <Link
                to="/admin/tools"
                className="flex items-center justify-center gap-2 text-white/40 hover:text-white text-[10px] font-black uppercase tracking-[0.2em] pt-4 transition-colors"
              >
